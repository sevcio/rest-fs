...
var fileserver = function (app, path_prefix) {
    if (!app) {
        throw new Error('express app required');
    }

    path_prefix = path_prefix.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&");

    app.use(require('express-domain-middleware'));

    app.use(function (req, res, next) {
        var domainRegex = new RegExp("^" + path_prefix + ".*$");

        if (domainRegex.test(decodeURI(url.parse(req.url).pathname))) {
            next();
        } else {
